...
In this situation we need to mock getCurrentPosition function because it is not
working in our node testing environments as it should in the browser

act is for side effect if we want to change state of external apis

### example of mocking function is in extra 2 - could be really useful!!!
...
testing are _WAY faster and WAY more capable when run in node_. Most of the
time, you can mock browser APIs for your tests without losing too much
confidence. However, if you are testing something that really relies on browser
APIs or layout (like drag-and-drop) then you may be better served by writing
those tests in a real browser (using a tool like [Cypress](https://cypress.io)).
